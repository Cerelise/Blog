  ...
  methods: {
    ...
    setUserToGroup() {
      ...
      if (userlist.length == 0) {
        swal("错误", "没有选择用户", "error");
        return;
      }
      // 提交用户组分配
      axios({
        url: "http://127.0.0.1:9000/api/amae-group/",
        method: "post",
        data: Qs.stringify({
    ...
    getUserList() {
      axios({
        url: "http://127.0.0.1:9000/api/amae-userlist/",
        method: "get",
      }).then((res) => {
        console.log(res.data);
        ...
        this.$store.dispatch("checkUserPerm", checkInfo).then((res) => {
          if (res) {
            axios({
              ...
              if (res.data == "same_name") {
                swal("警告", "重复命名", "warning");
                return;
              }
              if (res.data == "ok") {
                this.getAllUserGroup();
